            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-slate-100">
                <div class="p-6">
                    ...
                    <div class="mb-6 flex justify-between items-center">
                        <form method="GET" action="{{ route('admin.mechanic.index') }}" class="relative w-full max-w-md">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama mekanik atau spesialisasi..." class="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-[#FF6B00] transition-colors text-sm">
                    ...
                    <a href="{{ route('admin.mechanic.create') }}" class="px-4 py-2 bg-[#FF6B00] text-white rounded-lg hover:bg-orange-600 transition-colors font-medium text-sm flex items-center gap-2">
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-50 border-y border-slate-200">
                                    <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Nama Mekanik</th>
                                    <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Spesialisasi</th>
                                    ...
                                    <tr class="hover:bg-slate-50 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div class="font-bold text-slate-800">{{ $mechanic->name }}</div>
                                                    <div class="text-sm text-slate-500">{{ $mechanic->phone ?? '-' }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-3">
                                                <a href="{{ route('admin.mechanic.show', $mechanic->id) }}" class="text-slate-400 hover:text-blue-500 transition-colors" title="Detail">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                                </a>
                                                <a href="{{ route('admin.mechanic.edit', $mechanic->id) }}" class="text-slate-400 hover:text-[#FF6B00] transition-colors" title="Edit">
                                                ...
                                                <form action="{{ route('admin.mechanic.destroy', $mechanic->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus mekanik ini?');">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    <tr>
                                        <td colspan="4" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center justify-center text-slate-500">
                                                ...
                                                <p class="text-base font-medium text-slate-600">Belum ada mekanik yang terdaftar</p>
                                                <p class="text-sm mt-1">Gunakan tombol "Tambah Mekanik" untuk memasukkan data tim mekanik bengkel.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
